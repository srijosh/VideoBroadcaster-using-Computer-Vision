...
class CustomSegmentationWithYolo():
    def __init__(self, erode_size=5, erode_intensity=2):
         ...
         self.background_image = cv2.imread("./static/defaultbackg.jpg")
         
    def generate_mask_from_result(self,results):
        for result in results:
            if result.masks:
                # get array results
                masks = result.masks.data # Shape: (num_detections, H, W)
                boxes = result.boxes.data  # Shape: (num_detections, 6) where last column is class index
                
                # extract classes
                clss = boxes[:, 5] # Extract class indices (last column in `boxes`)
