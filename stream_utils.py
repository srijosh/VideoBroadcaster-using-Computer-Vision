...
class Streaming(CustomSegmentationWithYolo):
    def __init__(self, in_source=None, out_source=None, fps=None, blur_strength=None, cam_fps=30, background="none"):
    ...
    def stream_video(self):
        ...
        if self.fps:
            if self.fps>self.original_fps:
                ...
                frame_interval = int(self.original_fps / self.fps) #if original fps is 20 and user wants 2 fps then frame_interval = 10 ie select every 10th frame
        else:
            frame_interval=1
        ...
        with pyvirtualcam.Camera(width=width, height=height, fps=self.fps) as cam:
            ...
            while self.running and cap.isOpened():
                ...
                cam.send(cv2.cvtColor(result_frame, cv2.COLOR_BGR2RGB)) # OpenCV uses BGR, D3D11 uses RGB where D3D11 is the backend of pyvirtualcam
                cam.sleep_until_next_frame() #sleep_until_next_frame is used to match the fps of the virtual camera with the fps of the input camera
